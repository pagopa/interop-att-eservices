...
export const HeaderskeychainMock = z.object({
  authorization: z.string().nullish(),
  "x-correlation-id": z.string().nullish(),
});

export type HeaderskeychainMock = z.infer<typeof HeaderskeychainMock>;

export const ParsedHeaderskeychainMock = z
  .object({
    correlationId: z.string().uuid(),
  })
  .and(AuthData);
export type ParsedHeaderskeychainMock = z.infer<
  typeof ParsedHeaderskeychainMock
>;

export const readHeaderskeychainMock = (
  req: Request
): ParsedHeaderskeychainMock | undefined => {
  try {
    const headers = HeaderskeychainMock.parse(req.headers);
