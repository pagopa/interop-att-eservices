...
test("POST Data Preparation API", async () => {
  const testResponse: UserModel[] | null =
    await DataPreparationService.saveList(requestPostDataPreparation);
...
test("GET GetAll Data Preparation API", async () => {
  const testResponse: UserModel[] | null =
    await DataPreparationService.getAll();
  let isCorrectCF = false;
  if (
    testResponse &&
    testResponse[0].soggetto.codiceFiscale === "ABCDEF01G02H3453"
  ) {
    isCorrectCF = true;
  }
