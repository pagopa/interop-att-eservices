...
import { Trial } from "../db/schema/trial.js";
import { getCheckValue } from "../utils/checkValues.js";
...
export const TrialRepository = {
    async insert(
        ...
        if (!check_id) throw new Error(`Unknown check name: ${checkName}`);
    ...
    async findByCorrelationId(correlationId: string) {
        return db.select().from(Trial).where(eq(Trial.correlation_id, correlationId));
    },
