...
export function mapSourceSubjectToDbSubject(
  ...
  return {
    ...
    address_id: sourceSubject?.address?.id ?? "",
  };
}
...
export function mapSourceAddressToDbAddress(
  sourceAddress: RequestAR003Type
): DbAddress {
  return {
    id: uuidv4(),
...
export function mapApiBodyToDbModels(
  ...
  try {
    const subject: DbSubject = mapSourceSubjectToDbSubject(subjectBody);
    ...
    } else if (subjectBody.address) {
      try {
        const newDbAddress = mapSourceAddressToDbAddress(subjectBody.address);
        addresses = [...addresses, newDbAddress];
      } catch (error) {
        logger.error(
          "Error during single address conversion:",
          error,
          subjectBody.address
        );
      }
    } else if (!subjectBody.address || !subjectBody.subject) {
      throw new Error("Missing address or subject in request body");
    }
    ...
    logger.error("Error during subject/address conversion:", error);
    throw error;
  }
}
...
export function mapApiBodyToDbModelsUpdate(
  ...
  try {
    ...
    const sourceAddress = Array.isArray(subjectBody.address)
    ...
    const addressMapped = mapSourceAddressToDbAddress(sourceAddress);
    const address = { ...addressMapped, id: addressId };
