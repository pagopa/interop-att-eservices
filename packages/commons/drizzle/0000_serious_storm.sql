...
BEGIN
  IF EXISTS (
    SELECT FROM information_schema.tables
    WHERE table_name = 'list_requests' AND table_schema = 'att'
  ) AND NOT EXISTS (
    SELECT FROM information_schema.table_constraints
    WHERE constraint_name = 'list_requests_purpose_id_purposes_id_fk' AND table_schema = 'att'
  ) THEN
    ALTER TABLE "att"."list_requests" ADD CONSTRAINT "list_requests_purpose_id_purposes_id_fk" FOREIGN KEY ("purpose_id") REFERENCES "att"."purposes"("id") ON DELETE cascade ON UPDATE no action;
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_indexes WHERE indexname = 'da_sdr_fk_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_indexes WHERE indexname = 'lr_submitted_req_id_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_indexes WHERE indexname = 'lr_purpose_id_idx'
  ) THEN
    CREATE INDEX "lr_purpose_id_idx" ON "att"."list_requests" USING btree ("purpose_id");
  END IF;
END $$;
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_indexes WHERE indexname = 'sdr_list_request_fk_idx'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT FROM pg_indexes WHERE indexname = 'sdr_subject_id_idx'
  ) THEN
