...
describe("dataPreparationHandshakeRepository", () => {
  ...
  describe("saveList", () => {
    ...
    it("should throw error if saving fails", async () => {
      ...
      await expect(
        dataPreparationHandshakeRepository.saveList(genericRequest, key)
  ...
  describe("findAllByKey", () => {
    it("should return parsed array when data is found in cache", async () => {
      const key = "testKey";
      const mockSavedData =
        '[{ pourposeId: "testPourposeId", apikey: "testApikey", cert: "testCert" }]';
      ...
      (parseJsonToHandshakeArray as ReturnType<typeof vi.fn>).mockReturnValue(
        mockParsedData
      );
  ...
  describe("findByPurposeId", () => {
    it("should return matching handshake model when found in cache", async () => {
      ...
      const result = await dataPreparationHandshakeRepository.findByPurposeId(
        key,
        pourposeId
      );
