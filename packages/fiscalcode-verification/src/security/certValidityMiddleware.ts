...
import { certNotValidError, requestParamNotValid } from "../exceptions/errors.js";
import { ZodiosRouterContextRequestHandler } from "@zodios/express";
import { createCertificateHash } from "../utilities/certificateUtility.js";

export const verifyCertValidity: ZodiosRouterContextRequestHandler<ExpressContext> = async (req, res, next) => {
  ...
  try {
    ...
    const handshake = await DataPreparationHandshakeService.getByPurposeId(appContext.authData.purposeId);
    ...
    logger.error(`Si è verificato un errore durante la verifica della validità del certificato: ${error}`);
    // Gestisci gli errori di verifica del certificato
    res.status(500).send("Si è verificato un errore durante la verifica della validità del certificato.");
  }
};
