...
class DataPreparationHandshakeService {
  ...
  public async saveList(
    ...
    try {
      ...
      const persistedHandshakeData = await dataPreparationHandshakeRepository.findAllByKey(this.key);
       if (!isCertUnique( persistedHandshakeData, handshakeData)){
        console.log ("il certificato inserito è associato ad un altro pourposeId")
        throw certNotValidError(
          `The certificate is not valid`
        );
        }
      //se è vuota, la salvo senza ulteriori controlli
      if (persistedHandshakeData == null || persistedHandshakeData.length === 0) {
        await dataPreparationHandshakeRepository.saveList(handshakeData, this.key);
      } else {
        ...
        await dataPreparationHandshakeRepository.saveList(allHandshake, this.key);
      }
      const response = await dataPreparationHandshakeRepository.findAllByKey(this.key);
  ...
  public async getAll(): Promise<HandshakeModel[] | null> {
    try {
      ...
      logger.error(`getAll [HANDSHAKE]: Errore durante il recupero della lista.`, error);
      throw error;
    }
  }
  ...
  public async deleteAllByKey(): Promise<number | null> {
    try {
      ...
      //const hash = generateHash([this.appContext.authData.purposeId]);
      const response = await dataPreparationHandshakeRepository.deleteAllByKey(this.key);
