...
import { makeApiProblemBuilder } from "pdnd-model";
  ...
  () => {
    ...
    > = async (req, res, next): Promise<unknown> => {
      try {
        ...
        const signatureToken = Array.isArray(req.headers['agid-jwt-signature']) ? req.headers['agid-jwt-signature'][0] : req.headers['agid-jwt-signature'];
        if (!signatureToken) {
...
export const verifyJwtPayload = (jwtToken: string, req: any): void => {
    ...
    if (!req.headers['content-type'] || !req.headers['content-encoding']) {
      logger.error(`The "content-type" or "content-encoding" value in header is required`);
      throw ErrorHandling.tokenNotValid();
    }
    ...
    for (const headerName of requiredSignatureHeaders) {
      const signedHeaderExists = signedHeaders.some((header: SignedHeader) => header && header[headerName]);
    ...
    const signedContentType = signedHeaders.find((header: SignedHeader) => header && header['content-type']);
    if (signedContentType!['content-type'] !== req.headers['content-type']) {
      logger.error(`The content-type '${req.headers['content-type']}' value in request header is different from the value in payload ${signedContentType!['content-encoding']}`);
      throw ErrorHandling.tokenNotValid();
    }

    const signedContentEncoding = signedHeaders.find((header: SignedHeader) => header && header['content-encoding']);
    if (signedContentEncoding!['content-encoding'] !== req.headers['content-encoding']) {
      logger.error(`The content-encoding '${req.headers['content-encoding']}' value in request header is different from the value in payload ${signedContentEncoding!['content-encoding']}`);
      throw ErrorHandling.tokenNotValid();
    }
    
    const signedDigest = signedHeaders.find((header: SignedHeader) => header && header['digest']);
    if (!signedDigest!['digest'].startsWith("SHA-256")) {
      logger.error(`The digest '${signedDigest!['digest']}' value in token payload is invalid`);
      throw ErrorHandling.tokenNotValid();
    }
    
    if (!req.headers['digest'] || !req.headers['digest'].startsWith("SHA-256")) {
      logger.error(`The digest '${req.headers['digest']}' value in token payload is invalid`);
      throw ErrorHandling.tokenNotValid();
    }
