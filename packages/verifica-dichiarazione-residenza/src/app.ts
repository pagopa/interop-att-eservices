...
console.log("config.skipInteroperabilityVerification" + config.skipInteroperabilityVerification);
...
router.get('/', (req, res) => {
  ...
  const jwtSignatureString = Array.isArray(agidJwtSignature) ? agidJwtSignature[0] : agidJwtSignature;
  ...
  try {
    ...
    res.status(500).send('Si è verificato un errore durante la validazione del token');
  }
   return "done";
});
