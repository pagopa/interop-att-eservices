...
import { parseJsonToVerifyRequest, parseJsonToVerifyRequestArray, convertStringToVerifyRequest, convertStringToResponseRequestDigitalAddress } from '../../../src/utilities/jsonVerifyRequestUtilities.js';
...
describe('parseJsonToVerifyRequest', () => {
  ...
  it('should return a parsed object for valid JSON string', () => {
    const result = parseJsonToVerifyRequest(validVerifyRequestJsonString) as VerifyRequest;
    expect(result).toEqual(classToPlain(JSON.parse(validVerifyRequestJsonString)) as VerifyRequest);
  });
...
describe('parseJsonToVerifyRequestArray', () => {
  ...
  it('should return a parsed array for valid JSON string', () => {
    const result = parseJsonToVerifyRequestArray(validVerifyRequestArrayJsonString) as VerifyRequest[];
    expect(result).toEqual(JSON.parse(validVerifyRequestArrayJsonString).map((item: any) => classToPlain(item) as VerifyRequest));
  });
...
describe('convertStringToVerifyRequest', () => {
  ...
  it('should throw an error for invalid JSON string', () => {
    expect(() => convertStringToVerifyRequest(invalidJsonString)).toThrow(SyntaxError);
  });
});

describe('convertStringToResponseRequestDigitalAddress', () => {
  it('should return a parsed object for valid JSON string', () => {
    const result = convertStringToResponseRequestDigitalAddress(validResponseRequestDigitalAddressJsonString);
    expect(result).toEqual({
  ...
  it('should throw an error for invalid JSON string', () => {
    expect(() => convertStringToResponseRequestDigitalAddress(invalidJsonString)).toThrow(SyntaxError);
  });
});
